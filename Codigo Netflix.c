...
void exibir_favoritos(FILE *filme_file, FILE *user_file);
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 5:
                ...
                break;
            case 6:
                exibir_favoritos(filme_file, user_file);
                break;
            case 7:
...
void exibir_menu() {
    ...
    printf("6. Exibir Favoritos de um Usuário\n");
    printf("7. Sair\n");
}
...
void exibir_favoritos(FILE *filme_file, FILE *user_file) {
    ...
    for (int i = 0; i < usuario.num_favoritos; i++) {
        while (fread(&filme, sizeof(Filme), 1, filme_file)) {
            if (filme.id == usuario.filmes_favoritos[i]) {
                printf("- %s (%s)\n", filme.filme, filme.genero);
                break;
            }
        }
        fseek(filme_file, 0, SEEK_SET);
